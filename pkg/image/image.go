		...
		case req := <-c.pullRequests:
			...
			if err != nil {
				...
				go func() {
					...
					if err != nil {
						log.Error(err, "Error copying image")
						return
					}
...
func (c *LocalCache) retryPullImage(ctx context.Context, ref string) error {
	...
	for i := 0; i < maxRetries; i++ {
		...
		log.Error(err, "image couldn't be pulled")
		errs = append(errs, fmt.Errorf("trial %d of image pull failed with: %w ", i+1, err))
	}

	return fmt.Errorf("exceeded max retries, image pull failed with error(s): %v", errs)
}
...
func (c *LocalCache) pullImage(ctx context.Context, ref string) error {
	...
	if srcDigest != copiedDigest {
		if err = c.store.Delete(ctx, ref); err != nil {
			return fmt.Errorf("error deleting image from local oci store %s: %w", ref, err)
		}
		return fmt.Errorf("image digest verification failed for image %s: source digest %s, copied digest %s", ref, srcDigest, copiedDigest)
	}
	return nil
}

var ErrImagePulling = errors.New("image pulling")
